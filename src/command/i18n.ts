...
import { StatusBarItem } from '../i18n/statusBarItem';
...
export function createI18nCommand(CTX: vscode.ExtensionContext) {
    ...
    const handleI18nStart = () => {
        ...
        if (isI18nReay) {
            statusBarItem.fastBarItem();
            return vscode.window.setStatusBarMessage('ctools.i18n 已启动', 2000);
        }
    ...
    async function readI18nOptionsfiles() {
        if (statusBarItem) statusBarItem.loadingBarItem();
        try {
            const options = await getI18nOptionsConfiguration();
